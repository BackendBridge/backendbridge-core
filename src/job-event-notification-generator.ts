...
export function generateLaravelJobsEventsNotifications(contract: ApiContract, outPath: string): string[] {
  ...
  // Queue configuration
  const queueConfig = laravelQueueConfig(groups);
  const queueConfigPath = path.join(outPath, "config", "queue.generated.php");
  ensureDir(path.join(outPath, "config"));
  fs.writeFileSync(queueConfigPath, queueConfig, "utf8");
  generated.push(queueConfigPath);

  // .env queue example
  const envQueue = `# Queue configuration (merge into your .env)\nQUEUE_CONNECTION=redis\nREDIS_HOST=127.0.0.1\nREDIS_PASSWORD=null\nREDIS_PORT=6379\n\n# Or use database driver (simpler, no Redis needed)\n# QUEUE_CONNECTION=database\n\n# Run workers:\n# php artisan queue:work --queue=default,high\n# php artisan horizon  # if using Laravel Horizon\n`;
  const envQueuePath = path.join(outPath, ".env.queue.example");
  fs.writeFileSync(envQueuePath, envQueue, "utf8");
  generated.push(envQueuePath);

  return generated;
}

function laravelQueueConfig(groups: ResourceGroup[]): string {
  const jobList = groups.map((g) => `    // \\App\\Jobs\\Generated\\Process${g.name}Job::dispatch($id);`).join("\n");
  return `<?php

/**
 * Queue configuration hints — generated by BackendBridge.
 * Merge relevant parts into your config/queue.php
 */
...
return [
    ...
    // Generated jobs (dispatch from controllers/listeners):
${jobList}
];
`;
}
...
export function generateSymfonyJobsEventsNotifications(contract: ApiContract, outPath: string): string[] {
  ...
  // Messenger configuration YAML
  const messengerConfig = symfonyMessengerConfig(groups);
  const configDir = path.join(outPath, "config", "packages");
  ensureDir(configDir);
  const messengerConfigPath = path.join(configDir, "messenger.generated.yaml");
  fs.writeFileSync(messengerConfigPath, messengerConfig, "utf8");
  generated.push(messengerConfigPath);

  // .env messenger example
  const envMessenger = `# Messenger / async transport (merge into your .env)\nMESSENGER_TRANSPORT_DSN=redis://localhost:6379/messages\n\n# Or use AMQP:\n# MESSENGER_TRANSPORT_DSN=amqp://guest:guest@localhost:5672/%2f/messages\n\n# Or use Doctrine (simpler, no broker needed):\n# MESSENGER_TRANSPORT_DSN=doctrine://default\n\n# Run workers:\n# php bin/console messenger:consume async --limit=50\n`;
  const envMessengerPath = path.join(outPath, ".env.messenger.example");
  fs.writeFileSync(envMessengerPath, envMessenger, "utf8");
  generated.push(envMessengerPath);

  return generated;
}

function symfonyMessengerConfig(groups: ResourceGroup[]): string {
  const routingLines = groups
    .map((g) => `        App\\Message\\Generated\\${g.name}Message: async`)
    .join("\n");

  return `# Messenger configuration hints — generated by BackendBridge.
# Merge relevant parts into config/packages/messenger.yaml
